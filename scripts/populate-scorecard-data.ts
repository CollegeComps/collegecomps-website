 ...
 * 
 * Usage: npm run populate-scorecard-data
 */
...
interface ScorecardInstitution {
  id: number;
  'school.name': string;
...
async function fetchScorecardPage(page: number): Promise<ScorecardInstitution[]> {
  const fields = [
    'id',
    'school.name',
...
async function populateEarningsData(
  ...
  institutions: ScorecardInstitution[]
  ...
  for (const inst of institutions) {
    ...
    try {
      ...
      console.error(`Error updating earnings for unitid ${unitid}:`, error);
    }
  }
...
async function populateAdmissionsData(
  client: ReturnType<typeof createClient>,
  institutions: ScorecardInstitution[]
): Promise<{ updated: number; inserted: number }> {
  let updated = 0;
  ...
  for (const inst of institutions) {
    ...
    const satAvg = inst['latest.admissions.sat_scores.average.overall'];
    const actAvg = inst['latest.admissions.act_scores.midpoint.cumulative'];
    ...
    if (!admissionRate && !satAvg && !actAvg && !satMath25 && !actComp25) {
      continue;
    }
    ...
    try {
      ...
      // Acceptance rate is decimal (0.0 to 1.0), store as percentage by multiplying by 100
      // We'll use dummy values for applicants/admissions to preserve the rate
      ...
      // Log average scores for verification
      if (satAvg || actAvg) {
        // These are just for logging/verification
        console.log(`   Unit ${unitid}: Admission Rate=${admissionRate ? (admissionRate * 100).toFixed(1) + '%' : 'N/A'}, SAT=${satAvg || 'N/A'}, ACT=${actAvg || 'N/A'}`);
      }
    } catch (error) {
      console.error(`Error updating admissions for unitid ${unitid}:`, error);
    }
  }
...
async function main() {
  ...
  console.log(`   Found ${totalPages * PAGE_SIZE} institutions (approx)\n`);
