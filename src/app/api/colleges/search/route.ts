...
export async function GET(request: NextRequest) {
  ...
  try {
    const normalizedLevel = (['associates', 'bachelors', 'masters', 'doctorate', 'certificate'].includes(degreeLevel || ''))
      ? (degreeLevel as any) : undefined;

    const colleges = await cached(
      `college-search:${query.toLowerCase()}:${normalizedLevel || 'all'}`,
      2592000,
      async () => {
        ...
        return institutions.slice(0, 20).map((inst) => ({
          id: inst.id,
          unitid: inst.unitid,
          name: inst.name,
          state: inst.state,
